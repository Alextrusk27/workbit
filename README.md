...
- **CI** ([`ci.yml`](.github/workflows/ci.yml)) — on PRs to `develop` and `master`: `mvn verify` (unit tests plus Testcontainers integration tests), a backend image build without a push, frontend lint, tests and build; the `changes` job runs the backend and frontend jobs only when their paths changed.
- **Deploy** ([`deploy.yml`](.github/workflows/deploy.yml)) — on push to `master` (except frontend-only and documentation changes): tests, backend image to Yandex Container Registry, rollout to the VM over SSH ([`compose.prod.yml`](compose.prod.yml)) with a database dump beforehand and an automatic rollback to the previous image if the smoke fails; then an authenticated smoke with a live LLM call, a version tag and release notes, and a merge of `master` back into `develop` with the next snapshot version.
- **Deploy frontend** ([`deploy-frontend.yml`](.github/workflows/deploy-frontend.yml)) — on push to `master` that touches only the frontend or the legal documents in `docs/`: lint, tests and build, a preflight that checks the backend on prod matches `HEAD`, rsync of `dist` to the VM, smoke, then a merge of the rolled-out commit back into `develop`. A push that also touches backend paths is left to the full Deploy.
