...
- 💳 **Пакеты лимитов** — один общий баланс на пользователя, лимиты действуют 3 месяца с последней покупки, единый прайс на операцию (интервью, тренировка, эталонный ответ); разовая оплата через Робокассу, история операций в настройках.
...
- **Идемпотентные платежи** — вебхук Робокассы подтверждает платёж условным `UPDATE` (конкурентные ретраи не задваивают начисление), пакет лимитов начисляется в той же транзакции, а потерянные вебхуки добирает минутная джоба сверки через статусный API провайдера.
...
- [Биллинг](docs/billing-api.md) — баланс лимитов, история операций, оплата через Робокассу
